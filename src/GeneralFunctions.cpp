...
namespace gfuncs {
    ...
    void RemoveTrailingChar(std::string& s, char c) {
        int i = s.size() - 1;
        while (s[i] == c && i >= 0) {
            s.erase(s.end());
            i--;
    ...
    //papyrus treats all hex strings that start with 'f' as negative numbers
    int HexToIntPapyrus(std::string hex) {
        //RemoveLeadingChar(hex, '0');
        ...
        if (hex[0] == 'f') {
            iReturn = (std::pow(i16, P) * -1);
            I++;
            P--;
        }

        while (I < L) {
            std::string c = std::string{ hex.at(I) };
            int Value = hexDigits.find(c);
            if (Value == -1) {
                Value = 0;
            }

            iReturn += (Value * std::pow(i16, P));
            P--;
            I++;
        }
    ...
    void RefreshItemMenu() {
        //if (gui->IsItemMenuOpen()) { //now checked before called in plugin.cpp. Don't need to check twice.
    ...
    void SetQuestObject(RE::TESObjectREFR* ref, bool set) {
        if (!IsFormValid(ref)) {
            return;
        }
        ...
        if (aliasArray) {
            ...
            for (auto* akAlias : aliasArray->aliases) {
                if (akAlias) {
                    if (akAlias->alias) {
                        auto* owningQuest = akAlias->alias->owningQuest;
                        if (owningQuest) {
                            for (auto* alias : owningQuest->aliases) {
                                //gets around the error of using const alias in SetAliasQuestObjectFlag function
                                //without using const_cast
                                if (alias == akAlias->alias) {
                                    SetAliasQuestObjectFlag(alias, set);
                                    break;
                                }
                            }
                        }
    ...
    //Thanks to Meridiano, author of Papyrus Ini Manipulator for this.
    //Use this when you already know the container and item are valid, speeds up the process.
    std::int32_t GetItemCount_NoChecks(RE::TESObjectREFR* containerRef, RE::TESForm* item) {
        std::int32_t result = 0;
        using func_t = std::int32_t(*)(RE::TESObjectREFR*, RE::TESForm*, std::int64_t, std::int32_t);
        REL::Relocation<func_t> func{ REL::VariantID(56062, 56497, 0x9E4710) };
        result = func(containerRef, item, NULL, NULL);
